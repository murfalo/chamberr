...
def main():
    ...
    for atom_name, atom_type in list(amber_combined_params.atom_types.items()):
        # NOTE(MCA): Feel free to expand this check to other properties if you
        # get errors like "type.<your_property> is None" during conversion.
    ...
    # Export CHARMM .prm (parameters), .rtf (topologies), and an all-in-one
    # .str (stream file)
    ...
    print(
        f"Saving CHARMM-readable {leaprc_source}.prm, {leaprc_source}.rtf, "
        f"and {leaprc_source}.str in {output_dir}..."
    )
